...
- City agents: Task (`generalPurpose`). Prefer one message with multiple Task calls. Do not pass a model parameter.
- Ask the user conversationally for intake and the route-skeleton pick.
- `fast-flights` is optional. Without it, `flight_scan.py` falls back to a Google Flights link.
- Prefer `scripts/render_plan.py` for HTML. Upstream theme picture library is not vendored.
- Write working plans under `./trips/` unless the user names another path.
