...
## Choosing agent type (Cursor Task)

- Use **`generalPurpose`** for broad search + synthesis work (most research axes).
- Use **`explore`** only when the axis is about exploring an existing local codebase or file set — not for web research.
- Do not pass a `model` parameter (inherit). Launch independent axes in one message via multiple Task calls.
