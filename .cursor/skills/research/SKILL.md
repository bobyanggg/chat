---
name: research
description: Conduct preliminary research on a topic and generate a research outline (items + fields). Use when starting academic research, benchmarks, technology selection, competitor analysis, or when the user invokes /research.
...
- For web supplementation, use the Task tool (`generalPurpose`) with the web-researcher prompt below, or do WebSearch/WebFetch yourself for smaller topics.
- Before searching, load strategy modules from [references/web-search-modules/](references/web-search-modules/) as described in [references/web-search-agent.md](references/web-search-agent.md).
...
Output {step1_output}, ask user to confirm:
- Need to add/remove items?
- Does field framework meet requirements?

### Step 2: Web Search Supplement
Ask user for time range (e.g., last 6 months, since 2024, unlimited).
...
Launch 1 web-researcher Task (background), **Prompt Template**:
```python
prompt = f"""## Task
...
Before launching the Task, prepend instructions to read `.cursor/skills/research/references/web-search-agent.md` and load relevant modules under `.cursor/skills/research/references/web-search-modules/`.

### Step 3: Ask User for Existing Fields
Ask if user has an existing field definition file; if so read and merge.
